...
import { BlueGreenMoviePairPoints } from '../../src/material/AwardCards/BlueGreenMoviePairPoints'
...
describe('BlueGreenMoviePairPoints', () => {
  test.for([
    {
      playerMovies: [
    ...
    'Given archived movies with $numberOfBlueMovies blue movie(s) and $numberOfGreenMovies green movie(s), BlueGreenMoviesPairPoints should award $expectedPoints',
    ({ playerMovies, expectedPoints }) => {
      ...
      // When
      const awardedPoints = BlueGreenMoviePairPoints(playerMovieArchiveMaterial, playerTheaterTilesMaterial, guestCountsByColor, 6)
